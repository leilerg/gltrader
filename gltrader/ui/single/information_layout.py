...
class InformationLayout(GridLayout):
    ...
    def __init__(self, market=None, **kwargs):
        """
        Sets up screen to display and refresh information from market by 
        binding "market" property to refresh method that pulls from market currently assigned to property
        """
    ...
    def refresh(self, obj=None, market=None):
        ...
        if self.market.upToDate:
            ...
            if "available" not in self.widgets:
                self.widgets["available"] = Label(text=f'{self.market.data.balance["Available"]:.8f}')
                self.add_widget(self.widgets["available"])
            else:
                self.widgets["available"].text = f'{self.market.data.balance["Available"]:.8f}'
        else:
            if "name" not in self.widgets:
                self.widgets["name"] = Label(text=self.market.name+" (NOT CURRENT)")
                self.add_widget(self.widgets["name"])
            else:
                self.widgets["name"].text = self.market.name+" (NOT CURRENT)"
